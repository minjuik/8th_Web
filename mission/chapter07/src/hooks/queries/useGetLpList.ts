...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
  return useQuery({
    ...
    queryFn: () =>
      ...
      // 10분 동안 사용되지 않으면 해당 캐시 데이터가 삭제되며, 다시 요청 시 새 데이터를 받아온다.
      ...
      // retry: 3, -> 이건 App.tsx에 defaultOption으로 넣어주었음!
      ...
      // keepPreviousDate: 파라미터가 변경될 때 이전 데이터를 유지하여 UI 깜빡임(Flicking)을 줄여준다
