...
import { ResponseLpDto, Likes } from "../../types/lp";
...
function usePostLike() {
  return useMutation({
    ...
    onMutate: async (lp) => {
      // 1. 이 게시글에 관련된 쿼리 취소 (캐시된 데이터를 새로 불러오는 요청)
      ...
      // 3. 게시글 데이터 복사해서 NewLpPost라는 새로운 객체를 만들거임
      // 복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전 상태로 되돌리기 위해서임
      ...
      // 이렇게 하면 UI가 바로 업데이트 됨, 사용자가 변화를 확인할 수 잇다.
